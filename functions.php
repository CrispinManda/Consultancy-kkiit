...
function redirect_admin_to_dashboard() {
    ...
    if (user_can($user, 'manage_options')) { // Check if the user has admin capabilities
        return home_url('/dashboard'); // Replace '/dashboard' with the desired URL
    }

    return home_url();
}

add_filter('login_redirect', 'redirect_admin_to_dashboard');


function displayRegisteredPeople(){
    global $wpdb;

    $contact_table = $wpdb->prefix.'registrations';
